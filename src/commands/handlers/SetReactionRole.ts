...
export default class SetReactionRole implements CommandHandler {
    data = new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName("emoji_name").setDescription("Name of the emoji to use for the reaction").setRequired(true),
        )
        .addRoleOption((option) =>
            option.setName("role").setDescription("Role to use for the reaction").setRequired(true),
        )
        .addStringOption((option) =>
            option
                .setName("message_id")
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const messageId =
            interaction.options.getString("message_id", false) ?? (await this.getIdOfLastMessage(interaction));
        ...
        if (existingReactionRole !== null) {
            await interaction.reply({
                embeds: [new ErrorEmbed("reaction role already exists for this emoji.")],
                flags: [MessageFlags.Ephemeral],
            });
    ...
    private async getIdOfLastMessage(interaction: ChatInputCommandInteraction): Promise<string> {
