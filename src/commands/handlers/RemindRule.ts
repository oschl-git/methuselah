...
export default class RemindRule implements CommandHandler {
    data = new SlashCommandBuilder()
        ...
        .addIntegerOption((option) =>
            option.setName("number").setDescription("Number of the rule").setRequired(true),
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        if (number < 1 || number > rules.length) {
            await interaction.reply({
                embeds: [new ErrorEmbed(`rule number must be between 1 and ${rules.length}.`)],
        ...
        await interaction.reply({
            content: `${prefix}*remember rule ${number}:*\n`,
