...
export async function getEmojiId(name: string, guild?: Guild): Promise<string> {
    ...
    if (!id) {
        id = (await client.application.emojis.fetch()).find((emoji) => emoji.name === name)?.id;
    }
...
export async function getEmojiString(name: string, guild?: Guild): Promise<string> {
    const emojiId = await getEmojiId(name, guild);

    return `<:${name}:${emojiId}>`;
}

export async function tryGetEmojiString(name: string, guild?: Guild): Promise<string | null> {
    try {
