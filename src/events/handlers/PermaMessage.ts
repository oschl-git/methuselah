...
export default class PermaMessage implements EventHandler<Events.MessageCreate> {
    ...
    async execute(message: Message): Promise<void> {
        ...
        if (permaMessageManager.isAwaitingPermaMessage(message.author.id, message.channelId)) {
    ...
    private async setPermaMessage(message: Message): Promise<void> {
        ...
        logger.info(`User ${message.author} set permamessage for channel ${message.channelId}`);
    }
    ...
    private async handleExistingPermaMessage(message: Message): Promise<void> {
        ...
        if (permaMessage.sentMessageId) {
            try {
                const sentMessage = await message.channel.messages.fetch(permaMessage.sentMessageId);
