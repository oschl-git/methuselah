...
export default class WelcomeMessage implements EventHandler<Events.GuildMemberAdd> {
    ...
    async execute(member: GuildMember): Promise<void> {
        ...
        for (const guildWelcomeChannel of guildWelcomeChannels) {
            const channel = member.guild.channels.cache.get(guildWelcomeChannel.channelId);

            if (!channel || !channel.isTextBased()) {
                continue;
            }

            const emoji = await emojiLoader.tryGetEmojiString("methuselah", member.guild);

            const quote = welcomeQuotes[Math.floor(Math.random() * welcomeQuotes.length)];

            await channel.send(`${emoji} *${quote}*\n**${member} just joined the server.**`);
