...
export default class Echo implements EventHandler<Events.MessageCreate> {
    ...
    async execute(message: Message): Promise<void> {
        ...
        if (!echoMessageManager.isAwaitingEchoMessage(message.author.id, message.channelId)) {
            return;
        }
        ...
        logger.info(`Echoed message from [@${message.author.username}] in channel ${message.channelId}`);
    }
}
